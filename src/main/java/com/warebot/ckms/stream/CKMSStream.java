...
public class CKMSStream<T extends Number & Comparable<T>> implements Stream<T> {
     ...
     */
    public void merge(List<T> buffer) {
        int start = 0;
    ...
    public List<T> getSamples() {
        LinkedList<Sample> samplesCopy = new LinkedList<Sample>(samples);
        List<T> samples = new ArrayList<T>(samplesCopy.size());

        for(Sample s: samplesCopy){
            samples.add(s.value);
        }
        return samples;
    }
